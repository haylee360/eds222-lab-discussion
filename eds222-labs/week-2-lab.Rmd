...
rootdir <- ("~/MEDS/fall/EDS-222/eds222-lab-discussion/")
...
Crab body size appears to be mildly positively correlated with latitude. That is, as latitude increases, body size increases.

Water temperature appears to be negatively correlated with latitude. That is, as latitude increases water temp decreases.

```{r}
# Scatterplot of crab size and latitude
ggplot(pie_crab) +
  geom_point(aes(x = latitude, y = size, color = size)) +
  theme(aspect.ratio = 1) # Correlation looks steeper!

# Scatter plot of water temp and latitude
ggplot(pie_crab) +
...
2.  Interpret your two coefficients, paying careful attention to units.
    ...
    The intercept of the OLS line is -3.62 mm which doesn't make biological sense because you can't have negative crabs. The slope is 0.4851 which means for every 0.4851 mm increase in crab size you increase 1 degree latitude (???)

3.  Is your estimate of $\beta_0$ directly interpretable? What might you be concerned about when interpreting $\beta_0$?

    No, you can't have negative size

```{r}
summary(lm(size ~ latitude, pie_crab))

# y ~ x, y as a function of x, y(x)
...
```{r}
# geom_smooth of crab size and latitude
ggplot(pie_crab, aes(x = latitude, y = size, color = size)) +
  geom_point() + 
  geom_smooth(method = lm, 
              formula = y ~ x, 
              se = FALSE) +
  theme_minimal()# Correlation looks steeper!

# geom_smooth of water temp and latitude
ggplot(pie_crab, aes(x = latitude, y = water_temp)) +
  geom_point() +
  geom_smooth(method = lm, 
              formula = y ~ x,
              se = FALSE) +
...
```{r}
fit <- lm(size ~ latitude, pie_crab)

pie_crab2 <- pie_crab %>% 
  mutate(size_residual = resid(fit),
         predicted_size = predict(fit))
```
