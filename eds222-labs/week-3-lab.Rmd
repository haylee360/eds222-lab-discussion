...
```{r}
fit_crab <- lm(size ~ latitude, data = pie_crab)
fit_water <- lm(water_temp ~ latitude, data = pie_crab)
...
```{r}
sse_c <- sum((resid(fit_crab))^2)

# This is the same thing as the residual
# sum((predict(fit_crab) - pie_crab$size)^2)

sst_c <- sum((pie_crab$size - mean(pie_crab$size))^2)

1 - (sse_c/sst_c)
```

```{r}
sse_w <- sum((resid(fit_water))^2)

sst_w <- sum((pie_crab$water_temp - mean(pie_crab$water_temp))^2)
...
```{r}
# Other ways to run the lm
lm(size ~ latitude, pie_crab) %>% 
  summary()

pie_crab %>% 
  lm(size ~ latitude, .) %>% # dot refers to what ever is on the left 
  summary()
```
...
The intercept tells us the estimated mpg for cars in 1999 is 23.4274 mpg. 

**Intercept is y-intercept with continuous, with categorical, intercept becomes the value of the reference level!!**

3.  What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words? 

The coefficient estimate for the 2008 cars tells us that cars in 2008 get approximately 0.0256 more mpg. 

**Beta1 represents the change relative to your reference level**

```{r}
hwy_fit <- lm(hwy ~ year_fct, mpg)
summary(hwy_fit)
```
4.  Does your model suggest anything about whether fuel efficiency has evolved over time? Why or why not?

This model suggests that between 1999 cars and 2008 vintage cars, fuel efficiency hasn't changed much. 
